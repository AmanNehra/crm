...
<div id="content">
  <form method="post">   
   ...
   <th valign="top">Subject:</th>
    ...
    <td></td>
     <th valign="top"><input type="submit" value="Search"  class="form-finish" name="submit" style=" float:left; margin-right:15px;" /></th>     <td><input type="button"  onclick="export_excel();" value="Export into Excel" class="form-finish"  style=" width:150px;float:left; margin-right:15px;"  name="Export into Excel" /></td>
	...
	function export_excel()
	{
	 ...
	 <form name="search_export" id="search_export" action="report_requirenment_chart_export.php" method="post">
	 <input type="hidden" name="subject_ept" id="subject_ept" class="search_textbox" value="<?php echo $subject; ?>"/>
	  </form>
	<!--  start page-heading -->
	<div id="page-heading">
	...
	*/
    if($user <=4 ) 
	{  
		$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE subject LIKE '%$subject%'";			
	}
	else
	{
	   $query = "SELECT COUNT(*) as num FROM $tbl_name WHERE subject LIKE '%$subject%' $salesman_area";		
	}
...
//echo $page;die;
    ...
    if($user <=4)
	{
	   $sql = "SELECT * FROM $tbl_name WHERE subject LIKE '%$subject%' ORDER BY id LIMIT $start, $limit";	 
	}
	else
	{
	   $sql = "SELECT * FROM $tbl_name WHERE subject LIKE '%$subject%' $salesman_area ORDER BY id LIMIT $start, $limit";	 
	}
