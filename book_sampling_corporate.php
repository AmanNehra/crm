...
$success="";
/*$query=mysql_query("SELECT code FROM corporate_list where id='$sid'");
...
$q="INSERT INTO book_sample (id_of_name,name, address, city,district, state, country,salseman_id,salesman_name,sampling_type,group1,series,relate,remarks,sampling_Date) VALUES ('$sid','$name','$address','$city', '$district','$state', '$country','$salesman_id','$salesman_name','$sampling_type','$group','$series','CORPORATE','$remarks','$sampling_Date' )";
  ...
  for($j=1; $j<=$len; $j++)
     {
	   ...
	   if($sampling_type!='BOOKS_GIVEN')
	    $q2=mysql_query("INSERT INTO book_sample_details (uid,salseman_id,teacher_id,relate,sampling_Date) VALUES('$uid','$salesman_id','$teacher_id','CORPORATE','$sampling_Date')")  or die( mysql_error());
		
	   else if(isset($book_id))	     
			$q2=mysql_query("INSERT INTO book_sample_details (uid,salseman_id,teacher_id,book_id,qty,relate,sampling_Date) VALUES('$uid','$salesman_id','$teacher_id','$book_id','$specimen_qty','CORPORATE','$sampling_Date')")  or die( mysql_error());
			if($qq && $q2){ 
			mysql_query("COMMIT"); }
			else {
...
<tr>
	...
	<?php //echo '<pre>'; print_r ($re_sample);?>
		...
		<tr>
		 <th valign="top">Visit Date:</th>
			<td><input type="text" name="sampling_Date" id="datepicker"  class="inp-form33" />
</td>
		</tr>
        <tr>
			<th valign="top">School Name:</th>
			<td><input type="text" name="name"  class="inp-form33" value="<?php echo $re_sample->name;?>"/>
</td>
    ...
    <th>Distt:</th>
		...
		<tr id="p_school_old4">
			...
			<select  id="sal_id"  style="width: 183px;;border: 1px solid;height: 30px;border-radius: 4px;" onchange="return salseman_id_name(this.value)" > 
		   <?php
			  ...
			  ?>
<option <?php //if($value['user']==$_SESSION['SESS_id']){ echo "selected"; } ?> value="<?php echo $value['id']; ?>"><?php echo $value['name']; ?></option>              			  		
			  <?php 
			     }   
              ?>
  </select></td><input type="hidden" name="salesman_id" id="salseman_id" value="<?php echo $rty ; ?>" />
                <input type="hidden" name="salesman_name" id="salseman_name" value="<?php echo $rty1 ; ?>" />
...
<option value="others">Others</option>
          ...
          <tr>
			<th>Group</th>
					<td><select name="group" id="group" onchange="sampleBookList();corporateList(<?php echo $corporate_id; ?>)" style="width: 183px;;border: 1px solid;height: 30px;border-radius: 4px;"> 
		...
		<?php
			  ...
			  ?>
					...
					<th>Series</th>
					<td><select name="series" id="series" onchange="sampleBookList();corporateList(<?php echo $corporate_id; ?>)" style="width: 183px;;border: 1px solid;height: 30px;border-radius: 4px;"> 
			<option value="" selected="selected"> Plz Select</option>
		<?php
	   ...
	   <tr id="tr2" style="display:none" ><td colspan="4">
		...
		<td valign="top" colspan="3">
		  <textarea class="inp-form33" style=" width:100%; height:50px" name="remarks"></textarea>
		 </td> 		
		</tr>
	<tr>
