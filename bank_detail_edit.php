...
function update()
 {
  $id = $_POST['id'];
	...
	$distict=strtoupper($_REQUEST['distict']);
  ...
  $query=mysql_query("SELECT city FROM location_maste_info_list where id=$city");
			    ...
			    $city=$value['city']; 
	$sq=mysql_query("UPDATE bank_details_list SET name='$name',address='$address',account='$account', city='$city',distict='$distict', state='$state', contact='$contact', email='$email',ifsc='$ifsc',branch='$branch',date_added='$date' where id='$id'") or die(mysql_error());
	//echo'</pre>';print_r($sq);die;
	if($sq){
	     header('location:bank_detail_listing.php');
	    }
		else{ header('location:bank_detail_edit.php?id='.$id);}
 }
//end of update

//For fing unique field 
$result=mysql_query("SELECT account FROM bank_details_list WHERE id= '$id' ") or die(mysql_error());
...
if($row['account']==$account)
  ...
  {
   $query=mysql_query("SELECT account FROM  bank_details_list WHERE id!= '$id'");
...
$result=mysql_query("SELECT * FROM bank_details_list WHERE id= '$id' ") or die(mysql_error());
$row = mysql_fetch_array($result);
...
<tr>
	...
	<table border="0" cellpadding="0" cellspacing="0"  id="id-form"> 
        <tr>
        <th valign="top" style="height: 22px">Name:</th>
			<td style="height: 22px"><input type="text" class="inp-form" name="name" id="name" value="<?php echo $row['name']; ?>" required /></td>
         <th valign="top" style="height: 22px">Account:</th>
			<td style="height: 22px"><input type="text" class="inp-form" name="account" id="account" value="<?php echo $row['account']; ?>"  required/></td>  
	   </tr>
       ...
       <th valign="top">Contact No:</th>
         ...
         <th valign="top">Email ID:</th>
			<td><input type="email" class="inp-form" multiple name="email" id="email" value="<?php echo $row['email']; ?>"required/></td>   
	   </tr>
       <tr>
         <th valign="top">IFSC Code:</th>
			<td><input type="text" class="inp-form" name="ifsc" id="ifsc" value="<?php echo $row['ifsc']; ?>" required /></td>
         <th valign="top">Branch Code:</th>
			<td><input type="text" class="inp-form" name="branch" id="branch"value="<?php echo $row['branch']; ?>" required /></td>   
	   </tr>
