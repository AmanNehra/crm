...
/*$query=mysql_query("SELECT book_alias,isbn FROM  item_master_list");
  ...
  if($f!=1)
   {
*/  $sql=mysql_query("INSERT INTO expense_ent (head, designation,relate,amount,date1) VALUES ('$head', '$designation','$relate','$amount',now())")or die(mysql_error());

$sql1=mysql_query("INSERT INTO expense_ent_new (head, designation,relate,amount,date1) VALUES ('$head', '$designation','$relate','$amount',now())");
...
<div id="page-heading">
  <h1>Add New Expense Head Master Information</h1>
	...
	<td>
      ...
      <?php $resultdesi=mysql_query("SELECT head FROM expense_list") or die(mysql_error());
            while ($resultdeg = mysql_fetch_array($resultdesi))   { ?>
			  <option VALUE="<?php echo $resultdeg['head']; ?>"><?php echo $resultdeg['head']; ?></option> 
			<?php } ?>   </select>			
			</td>
      ...
      <?php $resultdesi=mysql_query("SELECT designation FROM other_designation_list order by id") or die(mysql_error());
            while ($resultdeg = mysql_fetch_array($resultdesi))   { ?>
			  <option  VALUE="<?php echo $resultdeg['designation']; ?>"><?php echo $resultdeg['designation']; ?></option> 
			<?php } ?>   </select>
            </td> 
	   ...
	   <th valign="top">Amount:</th>
			<td><input type="text" class="inp-form" name="amount" id="amount" required/></td>
       ...
       <!--<tr>      
			...
			<td><input type="text" class="inp-form" name="specification" id="specification" value=""  required/></td>         
	   </tr> -->
	<tr>
		<th>&nbsp;</th>
		<td valign="top">
