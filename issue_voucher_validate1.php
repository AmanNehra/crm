...
$qty=($_REQUEST['qty'.$j]);
$approved=($_REQUEST['approved'.$j]);//Approved Quantity

$query="UPDATE all_voucher SET qty='$approved',salesman_qty='$qty' WHERE uid='$id' AND book_id='$book_id' AND relate='1' ";
$q1=mysql_query($query)  or die( mysql_error());

}
$query="UPDATE issue_voucher SET status='1',approved_by='$usname' WHERE id='$id' ";
$q2=mysql_query($query) or die( mysql_error());

if($q1 && $q2)
  header("location:issue_voucher_listing_validate1.php");
}

$query=mysql_query("SELECT * FROM issue_voucher WHERE id='$id'");
$data=mysql_fetch_array($query);

$query1=mysql_query("SELECT * FROM all_voucher WHERE (uid='$id') AND (relate=1) ");
?>
<!-- start content-outer -->
...
<div id="page-heading">
  <h1>Specimen Voucher Validate1 </h1>  
</div>
        ...
        <tr>
         ...
         <tr >
		  ...
		  <input type="hidden" name="book_id_<?php echo $i; ?>" value="<?php echo $row['book_id'];?>"  class="textsmall"/>		
			...
			<td style="margin-left:20px;" ><input type="text" name="qty_<?php echo $i; ?>" value="<?php echo $row['qty'];?>" class="textsmall" readonly=""/></td>
			<td style="margin-left:20px;" ><input type="text" name="approved_<?php echo $i; ?>" value="<?php echo $row['qty'];?>" class="textsmall" /></td>
		 </tr>
		 <?php $i+=1; } ?>
		</table>
	    </td></tr>
	<tr>
		<th><input type="hidden" name="len" value="<?php echo $i;?>" /></th>
		<td valign="top">
