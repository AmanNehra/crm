...
<tr>
			...
			<td ><input type="text" name="book_name_<?php echo $i; ?>" value="<?php echo $value['item_name'];?>" class="textbig" /></td>
			<td  ><input type="text" name="class_<?php echo $i; ?>" value="<?php echo $value['class'];?>" class="textsmall"/></td>
			<td style="margin-left:20px;" ><input type="text" name="qty_<?php echo $i; ?>" value="<?php echo $quantity ;?>" class="textsmall" /></td>
			<td style="margin-left:20px;" ><input  type="hidden" name="price_<?php echo $i; ?>" value="<?php echo $value['price'] ;?>" class="textsmall" /></td>					
 </tr>
<?php $i+=1; } ?>
...
$query12=mysql_query("SELECT id FROM item_master_list WHERE (`subject`='$group') OR (`series`='$series')");//For select all item name from item table
...
while($val=mysql_fetch_array($query)){    
   ...
   while($val1=mysql_fetch_array($query1)){
		...
		$bal_qty=$total_issue_book-$total_return_book-$total_school_book_sampling-$total_teacher_book_sampling-$total_corporate_book_sampling-$total_department_book_sampling-$total_chain_book_sampling-$total_member_chain_book_sampling-$total_contact_person_book_sampling;//for balance quatity of books
       if($exist==1){
?>


<tr> 
			<td ><input type="checkbox" name="book_id_<?php echo $i; ?>" value="<?php echo $book_id;?>" class="textsmall" readonly=""/></td>			
			<td ><input type="text" name="" value="<?php echo $book_name;?>" class="textbig"  style="width: 180px;" readonly=""/></td>
			<td  ><input type="text" name="" value="<?php echo $class;?>" class="textsmall"/></td>
			<td style="margin-left:20px;" ><input type="text" name="" value="<?php echo $bal_qty;?>" class="textsmall" readonly=""/>
 <?php 
 $i+=1;
    }
