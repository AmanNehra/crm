...
function update()
 {
	...
	$sq12=mysql_query("UPDATE master_list SET route ='$name' where `route` LIKE '$name12'");
	$sq12=mysql_query("UPDATE corporate_list SET route ='$name' where `route` LIKE '$name12'");	
	//echo'</pre>';print_r($sq);die;
	
	if($sq){
	     header('location:route_no_listing.php');
	    }
		else{ header('location:rout_no_info_edit.php?id='.$id);}
