...
<div id="content">
  <form method="post">   
	...
	*/
	...
	{
	 if(isset($_SESSION['from']) && isset($_SESSION['to']) && isset($_SESSION['salesman_id']))
	  $query = "SELECT COUNT(DISTINCT id_of_name,relate) as num FROM $tbl_name WHERE status=1 AND salseman_id IN ($salesman_ids) AND date BETWEEN '$from' AND '$to' AND salseman_id='$salesman_id' AND relate IN('SCHOOL','TEACHER','CORPORATE','CONTACT_PERSON','CHAIN','MEMBER','DEPARTMENT')";
	 else
	  $query = "SELECT COUNT(DISTINCT id_of_name,relate) as num FROM $tbl_name WHERE status=1 AND salseman_id IN ($salesman_ids) AND relate IN('SCHOOL','TEACHER','CORPORATE','CONTACT_PERSON','CHAIN','MEMBER','DEPARTMENT')";         
...
//echo $page;die;
	...
	{ 
     ...
     $query1 = " SELECT * FROM book_sample WHERE status=1 $searchArea ";
	}
	else
	{
	 if(isset($_SESSION['from']) && isset($_SESSION['to']) && isset($_SESSION['salesman_id']))
	   $sql = "SELECT DISTINCT id_of_name,relate FROM $tbl_name WHERE status=1 AND salseman_id IN ($salesman_ids) AND date BETWEEN '$from' AND '$to' AND salseman_id='$salesman_id' AND relate IN('SCHOOL','TEACHER','CORPORATE','CONTACT_PERSON','CHAIN','MEMBER','DEPARTMENT') ORDER BY relate desc LIMIT $start, $limit ";
	 else
	   $sql = "SELECT DISTINCT id_of_name,relate FROM $tbl_name WHERE status=1 AND salseman_id IN ($salesman_ids) AND relate IN('SCHOOL','TEACHER','CORPORATE','CONTACT_PERSON','CHAIN','MEMBER','DEPARTMENT') ORDER BY relate desc LIMIT $start, $limit";        
