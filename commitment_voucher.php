...
//echo '<pre>'; print_r($_REQUEST); die();
	...
	$q="select c_no from commitment_voucher order by id desc LIMIT 1";
   $r=mysql_query($q);
...
$target_file = $target_dir . basename(time().$_FILES["attachment"]["name"]);
 if (move_uploaded_file($_FILES["attachment"]["tmp_name"], $target_file)) {
		...
		$q1=mysql_query("INSERT INTO commitment_voucher (c_no,c_date,c_year,ref,ref_date,pay_mode,school,principal,address,contact_school,given,contact_teacher,designation_teacher,corporate,c_persion,c_address,c_contact,commited_by,remarks,gross_amount,book_discount,net_amount,attachment,gift_pad,gift_percent,total_discount) VALUES('$c_no','$c_date','$c_year','$ref','$ref_date','$pay_mode','$school','$principal','$address','$contact_school','$given','$contact_teacher','$designation_teacher','$corporate','$c_persion','$c_address','$c_contact','$commited_by','$remarks','$gross_amount','$book_discount','$net_amount','$attachment','$gift_pad','$gift_percent','$total_discount')") or die( mysql_error());
		if($q1){
		 mysql_query("COMMIT");	
		 header("location:commitment_voucher_listing.php");
		 }
		 else
...
<tr>
   ...
   if($row!=0)
   {
   	...
   	$new_num = "C00001";
        ...
        <td width="17%"> Auto generate
        	<!-- <input type="text" name="c_no" class="inp-form" value="<?php echo $new_num;  //echo "C".$com_no; ?>"/>-->
        	</td>
	
    <th width="27%">Commitment Date</th>
        <td width="14%"><input name="c_date" type="text" class="inp-form"/ id="datepicker1" value="<?php
echo date("Y-m-d"); 
?>"></td>
		</tr>
	   <tr> 
		...
		<?php
		 ...
		 $current_mont=date('m');	
		 if($current_mont>03){ 
		 for($k=2014; $k<$current_year+1; $k++){
		  $c=$k;
		  $d=$c+1;		  
		?>
          <option value="<?php echo $c."-".$d;?>"><?php echo $c."-".$d;?></option>
        <?php } } 
	     else {
		...
		<option value="By Telephonic">By Telephonic</option>
		</select>
		</td>
	   </tr>
       ...
       <th width="27%">Refrence Date</th>
        <td width="14%"><input name="ref_date" id="datepicker" type="text" class="inp-form"/></td> 
	  ...
	  <tr>
        ...
        <?php
              ...
              ?>
  </select></td> <input type="hidden" name="school" id="school" />       
        <th width="10%">Principal</th>
        <td width="6%"><input name="principal" id="principal" type="text" class="inp-form" readonly=""/></td>
	 </tr>
	  <tr>
        <th width="26%">Address</th>
        <td width="17%"><input name="address"  id="address"type="text" class="inp-form" readonly=""/></td>         
        <th width="10%">Contact No. </th>
        <td width="6%"><input name="contact_school" id="contact" type="text" class="inp-form" readonly=""/></td>
	  </tr>	  
	  <tr>
	   ...
	   <input type="hidden" name="given" id="given1" />
	</td>
	
        <th width="27%">Contact No. </th>
        <td width="14%"><input name="contact_teacher" id="contact_teacher" type="text" class="inp-form" readonly=""/></td> 
      </tr>
	  <tr>
        <th width="10%">Designation</th>
        <td width="6%"><input name="designation_teacher" id="designation_teacher" type="text" class="inp-form" readonly=""/></td>	
       ...
       </div><input type="hidden" name="c_persion"  id="c_persion" class="inp-form"/></td>
        
	  </tr> 
	    <th width="26%">Address</th>
        <td width="17%"><input name="c_address" id="c_address" type="text" class="inp-form"/></td>        
        <th width="10%">Contact No. </th>
        <td width="6%"><input name="c_contact" id="c_contact" type="text" class="inp-form"/></td>
	  ...
	  <tr>
		...
		<?php
				...
				{
					$query=mysql_query("SELECT name,user FROM department_list WHERE department='SALES' ORDER BY name");
					echo "<option value='' selected='selected'> Plz Select</option>";
				}
				else
				{
			    	$query=mysql_query("SELECT name,user FROM department_list where `department`='SALES' and user=$rt ORDER BY name");
				}			    
			    while($value=mysql_fetch_array($query))
			      {
			  ?>			  
			     <option  value="<?php echo $value['name']; ?>" <?php if($value['user']==$_SESSION['SESS_id']){ echo "selected"; } ?>><?php echo $value['name']; ?></option>
			  <?php 
			     }   
              ?>
		</select>
		...
		<td ><textarea class="inp-form" style="height:40px; width:99%" name="remarks"></textarea></td>
	</tr>
	   ...
	   <th width="27%">Gross Amount</th>
        <td width="14%"><input name="gross_amount" type="text" id="gross" class="inp-form"/></td> 
		 <th>Book Discount&nbsp;&nbsp;(Fill)</th>			
		   <td><input type="text" name="book_discount" id="discount" onkeyup="return discountOnGross(this.value);"  class="inp-form"/></td>
						
	</tr>
	<tr>
	  
	    <th width="10%">Net Amount</th>
        <td width="6%"><input name="net_amount" id="net" type="text" class="inp-form" readonly=""/></td>
	  ...
	  <tr>         
        ...
        <td width="14%"><input type="text" name="gift_percent" id="gift_percent" onkeyup="return discountOnGift(this.value);" class="inp-form"  /></td>
		<th width="10%">Total Amount </th>
        <td width="6%"><input name="total_discount" id="total_discount" type="text" class="inp-form" readonly=""/></td>
	  </tr>
	  <tr>       
        	
		
		<th>Attach Document</th>
		<td><input type="file" name="attachment" class="inp-form" required /></td>	
	</tr>
