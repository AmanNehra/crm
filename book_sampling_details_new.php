...
<div id="content">
	...
	*/
	...
	if($user <=4 ) 
	{  
		$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE view = '0' $search_val";			
	}
	else
	{	
	$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE view = '0' $salesman_area  $search_val";	
	}
	...
	$targetpage = "book_sampling_details_new.php"; 	//your file name  (the name of this file)
	$limit = 10;
...
//echo $page;die;
    ...
    if($user <=4)
	{ 
	 $sql = "SELECT * FROM $tbl_name WHERE view = '0' $search_val ORDER BY id desc LIMIT $start, $limit";
	}
	else
	{
	 $sql = "SELECT * FROM $tbl_name WHERE view = '0' $salesman_area  $search_val ORDER BY id desc LIMIT $start, $limit";
	}	
  ...
  echo "<tr class='alternate-row'>";
				...
				$days = floor(($diff - $years * 365*60*60*24 - $months*30*60*60*24)/ (60*60*24));
				  ...
				  ?>
					...
					<?php //if($days<1){ ?> 
<a href="book_sampling_edit_new.php?id=<?php echo base64_encode(convert_uuencode($row['id']));?>" title="Edit" class="icon-1 info-tooltip"></a> <?php// } ?>
							
					
					<a href="delete_sampling_new.php?delete=SD&id=<?php echo base64_encode(convert_uuencode($row['id']));?>&type=<?php echo base64_encode(convert_uuencode($tbl_name));?>&t_page=<?php echo $targetpage; ?>" onClick="if(!confirm('Are you sure, you want to delete this Information ?')){return false;}" title="Delete" class="icon-2 info-tooltip" ></a>
