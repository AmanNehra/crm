...
$salesman_area=working_area_member_chain_school($userid);
...
<div id="content">
	...
	<td> 
         <select name="searchby1" class="search_by" >
				...
				<option <?php if($searchby1=="name") echo 'selected="selected"';?>VALUE="name">Chain Name</option>	
				<option <?php if($searchby1=="member") echo 'selected="selected"';?>VALUE="member">Member Name</option>						
	...
	<td> 
         <select name="searchby2" class="search_by">
		        <option value="" selected="selected">Plz Select</option> 			
				<option <?php if($searchby2=="name") echo 'selected="selected"';?>VALUE="name">Chain Name</option>	
				<option <?php if($searchby2=="member") echo 'selected="selected"';?>VALUE="member">Member Name</option>						
			...
			<div id="table-content">
				...
				<tr>
                   ...
                   <th class="table-header-repeat line-left"><a>Member Name</a></th>
					...
					<th class="table-header-repeat line-left minwidth-1"><a>Sampling</a>	</th>				
	...
	*/
	if($searchby1!="" && $search1!="" && $searchby2!="" && $search2!="")
	  {
	  	$search_val=" AND (s.$searchby1 LIKE '%$search1%') AND (s.$searchby2 LIKE '%$search2%') ";
	  }
	else if($searchby1!="" && $search1!="")
	  {
	  	$search_val="AND (s.$searchby1 LIKE '%$search1%')";  
	  }
	else
	  {
	  	$search_val="";
...
//echo $page;die;
	...
	if($user <=4)
	{ 
	 $sql = "SELECT s.*,c.name as schoolname FROM $tbl_name  as s inner join chain_school_list as c on s.sid=c.id WHERE status in ('0','1') $search_val ORDER BY id desc LIMIT $start, $limit";
	}
	else
	{
	 $sql = "SELECT s.*,c.name as schoolname FROM $tbl_name  as s inner join chain_school_list as c on s.sid=c.id Where s.status='1' $salesman_area $search_val ORDER BY s.id desc LIMIT $start, $limit";
	}
				 ...
				 /* $len=strlen($ag_id);
				  ...
				  <td><?php echo $count; ?></td>
					...
					<td><?php echo $row['email']; ?></td>
                     ...
                     <td align="center"><a href="book_sampling_member_chain_of_school.php?sid=<?php echo base64_encode(convert_uuencode($row['id']));?>&type=<?php echo base64_encode(convert_uuencode($tbl_name));?>" title="Teacher Master"><img src="images/sicon.png"/></a></td>
					<td class="options-width">
                    
					<a href="chain_school2_edit.php?id=<?php echo base64_encode(convert_uuencode($row['id']));?>&sid=<?php echo base64_encode(convert_uuencode($sid));?>&f=member" title="Edit" class="icon-1 info-tooltip"></a>
                  <?php  if($user==1 || $user==2) { ?>
					<a href="delete.php?type=member&id=<?php echo base64_encode(convert_uuencode($row['id']));?>&sid=<?php echo base64_encode(convert_uuencode($sid));?>" onClick="if(!confirm('Are you sure, you want to delete this Information ?')){return false;}" title="Delete" class="icon-2 info-tooltip" ></a>
				<?php } ?>
