...
function update()
 {
	...
	$sq=mysql_query("UPDATE master_list SET satoff='$name' where satoff='$name12'");
	
	$sq=mysql_query("UPDATE corporate_list SET dayoff='$name' where dayoff='$name12'");	
	
	//echo'</pre>';print_r($sq);die;
	if($sq){
	     header('location:saturday_off_listing.php');
	    }
		else{ header('location:saturday_off_info_edit.php?id='.$id);}
...
<div id="page-heading">
  <h1>Edit Saturday Off Master</h1>
...
<tr>
   ...
   <th valign="top">Date:</th>
			<td>
            <input type="text" class="inp-form" name="name" id="name"  value="<?php echo $row['name'];?>" required/>            
...
messages: {
    category:{
                 required: "Enter saturday off category",                
    },
     date: {
                required: "Enter date",
