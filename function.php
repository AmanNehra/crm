        ...
        if($user > 6)
          ...
          {        
            ...
            if($queryresult!="")
            {
                $query="";
                $data="";
                $query="SELECT DISTINCT salesman_id FROM  working_area AS wa  JOIN dan_users AS du ON du.id=wa.salesman_id AND du.user_type >= '$user' WHERE $queryresult ";		
        		$data=mysql_query($query) or die(mysql_error());
        		$user_ids="";
        		while($row=mysql_fetch_assoc($data) )
        			{
        ...
        if($user > 6)
          {
            $query="";
            $data="";
            $query="SELECT name FROM  department_list WHERE `user`='$id' ";		
    		$data=mysql_query($query) or die(mysql_error());
    		$user_ids="";
    		$row=mysql_fetch_assoc($data);
        ...
        else
          {
            ...
            if($user_ids)
            {
                $query="";
                $data="";
                $query="SELECT name FROM  department_list WHERE `user` IN( $user_ids ) ";
        		//echo $query;exit;
        		$data=mysql_query($query) or die(mysql_error());
        		$user_names="";
        		while($row=mysql_fetch_assoc($data) )
