 ...
 <form method="post">   
   ...
   <th valign="top">To Date:</th>
	...
	<tr>
      ...
      <th>City:</th>
	 <td><select name="city" id="city" class="inp-form-select"  > 
			<option value="" selected="selected"> Plz Select</option>
      <?php			    
   ...
   <script>
	function export_excel()
	{
	 ...
	 <form name="search_export" id="search_export" action="report_salesman_expence_export.php" method="post">
	...
	<!--  start page-heading -->
	<div id="page-heading">
		<h1>Salesman Area/Item wise Expence Report</h1>
	</div>
    <div style="width:100%;">
	...
	*/
	...
	if($_SESSION['from']!="" && $_SESSION['to']!="" && $_SESSION['salesman_id']!="" )
	...
	$targetpage = "report_salesman_expence.php"; 	//your file name  (the name of this file)
	$limit = 20;
...
if ($i % 2 == 0) {
  ...
  echo "<tr class='alternate-row'>";
                  ...
                  <?php
                    $searchArea="";
                    if( $_SESSION['state']!="" ){
    				      $searchArea .=" AND state='$state' ";
    				 }  
                    if($_SESSION['district']!="")
                     {
                        $searchArea .=" AND district='$district' ";
                     }
                    if($_SESSION['city']!="")
                     {
                        $searchArea .=" AND city='$city' ";
                     }
                    if($_SESSION['from']!="" && $_SESSION['to']!="" )
                     {
                        $searchArea .=" AND date BETWEEN '$from' AND '$to' ";
                     }
                    if( $_SESSION['salesman_id']!="")
                     {
                        $searchArea .=" AND salseman_id='$salesman_id' ";
                     } 
				  ...
				  if($user <=4)
                	{ 
                	 $query1 = " SELECT * FROM issue_voucher WHERE status=2 $searchArea ";
                	}
                	else
                	{
                	 $query1 = " SELECT * FROM issue_voucher WHERE status=2 $searchArea AND salseman_id IN ($salesman_ids)  ";
                	} 
				  ...
				  //For return Items
				   if($user <=4)
                	{ 
                	 $query1 = " SELECT * FROM return_voucher WHERE status=2 $searchArea ";
                	}
                	else
                	{
                	 $query1 = " SELECT * FROM return_voucher WHERE status=2 $searchArea AND salseman_id IN ($salesman_ids)  ";
                	}
