...
<div id="content">
	...
	<!--  start page-heading -->
	<div id="page-heading">
		<h1>All Marital  Information Listing</h1>
	</div>
    <div style="width:100%;">
      <?php if($user==1){ ?>
      <?php }?>
		...
		<div id="content-table-inner">
			...
			<div id="table-content">
				...
				<tr>
                   ...
                   <th class="table-header-repeat line-left"><a>Organization</a></th>
                    <th class="table-header-repeat line-left"><a>Marital status</a></th>
	...
	*/
	$query = "SELECT id FROM teacher_master_list where mstatus IN ('$date[0]','$date[1]','$date[2]') UNION ALL select id from corporate_sub where mstatus IN ('$date[0]','$date[1]','$date[2]') UNION ALL select id from department_list where mstatus IN ('$date[0]','$date[1]','$date[2]') UNION ALL select id from chain_school_sub where mstatus IN ('$date[0]','$date[1]','$date[2]') ";
	
	$total_pages = mysql_num_rows(mysql_query($query));	
	//$total_pages = $total_pages['num'];
	//print_r($total_pages);die;
	/* Setup vars for query. */
	$targetpage = "mstatus.php"; 	//your file name  (the name of this file)
	$limit = 10;
...
//echo $page;die;
	...
	/* Get data. */
	$sql = "SELECT 'teacher_master_list',uid,teacher,mstatus,city,district,state FROM teacher_master_list where mstatus IN ('$date[0]','$date[1]','$date[2]') UNION ALL select 'corporate_sub',uid,persion,mstatus,city,district,state from corporate_sub where mstatus IN ('$date[0]','$date[1]','$date[2]') UNION ALL select 'department_list', id,name,mstatus,city,district,state from department_list where mstatus IN ('$date[0]','$date[1]','$date[2]') UNION ALL select 'chain_school_sub',sid,member,mstatus,city,district,state from chain_school_sub where mstatus IN ('$date[0]','$date[1]','$date[2]') ";
