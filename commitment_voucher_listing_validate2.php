...
<div id="content">
	...
	<div id="page-heading" style="display:inline;width:70%; float:left">
		<h1>Commitment Voucher Listing Validate 2 </h1>		
	</div>   
	...
	*/
    if($user <=4 ) 
	{
     ...
     {
        $query = "SELECT COUNT(*) as num FROM $tbl_name WHERE status='1' AND `commited_by` IN ($salesman_name)";
	...
	$targetpage = "commitment_voucher_listing_validate2.php"; 	//your file name  (the name of this file)
	$limit = 10;
...
//echo $page;die;
    ...
    if($user <=4 ) 
	{
     ...
     {
        $sql = "SELECT * FROM $tbl_name WHERE status='1' AND `commited_by` IN ($salesman_name) ORDER BY id desc LIMIT $start, $limit";             
  ...
  echo "<tr class='alternate-row'>";
				 ...
				 /* $len=strlen($ag_id);
				  ...
				  ?>
					...
					<a href="commitment_voucher_approved2.php?id=<?php echo base64_encode(convert_uuencode($row['id']));?>" title="Edit" class="icon-1 info-tooltip"></a>
