...
{
	$w = " WHERE salseman='".strtoupper($_SESSION['SESS_fname'])."'";
	$w1 = " AND salseman='".strtoupper($_SESSION['SESS_fname'])."'";
	/*$w = " ";
	$w1 = " ";*/
}
else
...
<div id="content">
	...
	<!--  start page-heading -->
	<div id="page-heading" style="display:inline;width:42%; float:left">
		<h1>All Return Voucher Listing</h1>
	</div>
	<form name="search" action="#" method="post">
	<div style="display:inline;width:27%;float:left">
	  <table><tr>
		<th>Show List</th>
    ...
    <div class="addin" style="width:auto; float:right;">
    	<span><a href="return_voucher.php" style="color:#FFFFFF;">Add New Return Voucher</a></span>
    </div>
   ...
   <div style="width:90%; float:left; padding-left:10%;">
	...
	function export_excel()
	{
	 ...
	 <form name="search_export" id="search_export" action="return_voucher_listing_export.php" method="post">
	...
	*/
	...
	$tbl_name="return_voucher";		//your table name
	...
	*/
    ...
    if($w==" ")
      {
        $w_with_salesman="where `salseman_id` IN ($salesman_ids)";
      }
    else
      {
        $w_with_salesman="$w AND `salseman_id` IN ($salesman_ids)";
      }
    ...
    if($user <=4 ) 
	{
    	if($show_list!=""){
    	 if($show_list=="all")
    	   $query = "SELECT COUNT(*) as num FROM $tbl_name $w ";
    	  else if($show_list=="approved")
    	   $query = "SELECT COUNT(*) as num FROM $tbl_name WHERE (status='2') $w1";
    	  else if($show_list=="not approved")
    	   $query = "SELECT COUNT(*) as num FROM $tbl_name WHERE (status!='2') $w1";
    	}
    	else if($searchby1!="" && $search1!="" && $searchby2!="" && $search2!="")
    	$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE ($searchby1 LIKE '%$search1%') AND ($searchby2 LIKE '%$search2%') $w1";	
    	else if($searchby1!="" && $search1!="")
    	$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE ($searchby1 LIKE '%$search1%') $w1";	
    	else
    	$query = "SELECT COUNT(*) as num FROM $tbl_name $w";
   }
    else
    {
        if($show_list!=""){
    	 if($show_list=="all")
    	   $query = "SELECT COUNT(*) as num FROM $tbl_name WHERE (status='2')$w AND `salseman_id` IN ($salesman_ids)";
    	  else if($show_list=="approved")
    	   $query = "SELECT COUNT(*) as num FROM $tbl_name WHERE (status='2') $w1 AND `salseman_id` IN ($salesman_ids)";
    	  else if($show_list=="not approved")
    	   $query = "SELECT COUNT(*) as num FROM $tbl_name WHERE (status!='2') $w1 AND `salseman_id` IN ($salesman_ids)";
    	}
    	else if($searchby1!="" && $search1!="" && $searchby2!="" && $search2!="")
    	$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE ($searchby1 LIKE '%$search1%') AND ($searchby2 LIKE '%$search2%') $w1 AND `salseman_id` IN ($salesman_ids)";	
    	else if($searchby1!="" && $search1!="")
    	$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE ($searchby1 LIKE '%$search1%') $w1 AND `salseman_id` IN ($salesman_ids)";	
    	else
    	$query = "SELECT COUNT(*) as num FROM $tbl_name $w_with_salesman ";
    }
	...
	$targetpage = "return_voucher_listing.php"; 	//your file name  (the name of this file)
	$limit = 10;
...
//echo $page;die;
    ...
    if($user <=4 ) 
	{
    	if($show_list!=""){
    	  if($show_list=="all")
    	   $sql ="SELECT * FROM $tbl_name $w ORDER BY id desc LIMIT $start, $limit";
    	  else if($show_list=="approved")
    	   $sql = "SELECT * FROM $tbl_name WHERE (status='2') $w1 ORDER BY id desc LIMIT $start, $limit";	  
    	  else if($show_list=="not approved")
    	    $sql = "SELECT * FROM $tbl_name WHERE (status!='2') $w1 ORDER BY id desc LIMIT $start, $limit";
    	}
    	else if($searchby1!="" && $search1!="" &&  $searchby2!="" && $search2!="")
    	$sql = "SELECT * FROM $tbl_name WHERE ($searchby1 LIKE '%$search1%') AND ($searchby2 LIKE '%$search2%') $w1 ORDER BY id desc LIMIT $start, $limit";
    	else if($searchby1!="" && $search1!="")
    	$sql = "SELECT * FROM $tbl_name WHERE ($searchby1 LIKE '%$search1%') $w1 ORDER BY id desc LIMIT $start, $limit";
    	else
    	$sql = "SELECT * FROM $tbl_name $w ORDER BY id desc LIMIT $start, $limit";
    }
     else
     {
        if($show_list!=""){
    	  if($show_list=="all")
    	   $sql ="SELECT * FROM $tbl_name $w_with_salesman ORDER BY id desc LIMIT $start, $limit";
    	  else if($show_list=="approved")
    	   $sql = "SELECT * FROM $tbl_name WHERE (status='2') $w1 AND `salseman_id` IN ($salesman_ids) ORDER BY id desc LIMIT $start, $limit";	  
    	  else if($show_list=="not approved")
    	    $sql = "SELECT * FROM $tbl_name WHERE (status!='2') $w1 AND `salseman_id` IN ($salesman_ids) ORDER BY id desc LIMIT $start, $limit";
    	}
    	else if($searchby1!="" && $search1!="" &&  $searchby2!="" && $search2!="")
    	$sql = "SELECT * FROM $tbl_name WHERE ($searchby1 LIKE '%$search1%') AND ($searchby2 LIKE '%$search2%') $w1 AND `salseman_id` IN ($salesman_ids) ORDER BY id desc LIMIT $start, $limit";
    	else if($searchby1!="" && $search1!="")
    	$sql = "SELECT * FROM $tbl_name WHERE ($searchby1 LIKE '%$search1%') $w1 AND `salseman_id` IN ($salesman_ids) ORDER BY id desc LIMIT $start, $limit";
    	else
    	$sql = "SELECT * FROM $tbl_name $w_with_salesman ORDER BY id desc LIMIT $start, $limit";
    }        
  ...
  echo "<tr class='alternate-row'>";
				 ...
				 /* $len=strlen($ag_id);
				  ...
				  ?>
					...
					<a href="return_voucher_view.php?id=<?php echo base64_encode(convert_uuencode($row['id']));?>" title="Edit" class="icon-1 info-tooltip"></a>
                  <?php  if($user==1 || $user==2) { ?>
					<a href="delete.php?type=return&id=<?php echo base64_encode(convert_uuencode($row['id']));?>" onClick="if(!confirm('Are you sure, you want to delete this Information ?')){return false;}" title="Delete" class="icon-2 info-tooltip" ></a>
				<?php } ?>
				</td>
				<?php //}?>
