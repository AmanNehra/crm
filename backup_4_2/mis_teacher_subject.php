...
<div id="content">
	...
	<form name="search113" method="post">
		<h1><span style="float:left">Teacher Subject</span> <span style="float: left; margin-left:70px"><input type="text" name="search112" style=" border-radius:5px;height: 23px;width: 180px;" placeholder="Enter School Code" value="<?php if($_REQUEST['all']) echo" "; ?>"></span><span style="float:left; margin-left:25px;"><input type="submit" name="search111" value="Search" style=" border-radius:5px"></span> </h1>
		</form>

	</div>
    <div style="width:100%;">
       <div class="addin" style="width:auto; float:right;">
    	<span><a href="mis_teacher_subject.php" style="color:#FFFFFF;">Show All</a></span>
    </div>    
			...
			<div id="table-content">
				...
				<tr>
                   ...
                   <!-- <th class="table-header-repeat line-left"><a>Profile Image</a></th>-->
					...
					<th class="table-header-repeat line-left minwidth-1"><a>Contact Number</a></th> 
				...
				$query111=mysql_query("SELECT id FROM  `master_list` WHERE code='$search112'");
				$data111=mysql_fetch_assoc($query111);
				$search112=$data111['id'];
	/*
	...
	*/
	if($search112!=NULL)		
	  $query = "SELECT COUNT(*) as num FROM $tbl_name WHERE uid='$search112'";
	else
	...
	$targetpage = "mis_teacher_subject.php"; 	//your file name  (the name of this file)
	$limit = 10;
...
//echo $page;die;
	...
	if($search112!=NULL)
	{
	$sql = "SELECT * FROM $tbl_name WHERE (uid='$search112') ORDER BY id desc LIMIT $start, $limit";
	}
	else
...
if ($i % 2 == 0) {
  ...
  echo "<tr class='alternate-row'>";
				  ...
				  ?>
                    ...
                    <td><?php if($row['phone1']!="")echo $row['phone1']; if($row['mobile1']!="")echo ",".$row['mobile1']; if($row['mobile2']!="")echo ",".$row['mobile2']; ?></td>
					<td><?php foreach($subject as $s) { if($s!="")echo $s.","; }?></td>  
