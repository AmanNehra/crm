...
<div id="content">
	...
	<form name="search113" method="post">
		<h1><span style="float:left"> Expense Entitalment Listing</span> <span style="float: left; margin-left:70px"><input type="text" name="search112" class="searchboxs" value="<?php if($_REQUEST['all']) echo" "; ?>"></span><span style="float:left; margin-left:25px;"><input type="submit" name="search111" value="Search" class="searchboxs"></span>  </h1>
		</form>
    ...
    <div class="addin" style="width:auto; float:right;">
    	<span><a href="add_expense_ent_master.php" style="color:#FFFFFF;">Add New Expense Entitalment</a></span>
    </div> 
	 </div>
	<div class="addin" style="width:auto; float:right;">
    	<span><a href="expense_ent_listing.php" style="color:#FFFFFF;">Show All</a></span>
    </div>  
			...
			<div id="table-content">
				...
				<tr>
                   ...
                   <!-- <th class="table-header-repeat line-left"><a>Profile Image</a></th>-->
                    <th class="table-header-repeat line-left"><a>Head</a></th>
					<th class="table-header-repeat line-left minwidth-1"><a>Designation</a></th>
					<th class="table-header-repeat line-left minwidth-1"><a>Relate</a></th>                    					 			        <th class="table-header-repeat line-left minwidth-1"><a>Amount</a></th>
	...
	*/
	...
	$tbl_name="expense_ent";		//your table name
	...
	*/	
	if($search112!=NULL)
	{
	$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE (head LIKE '%$search112%') OR (designation LIKE '%$search112%') OR (relate LIKE '%$search112%') OR (amount LIKE '%$search112%')";
	}
	else
	...
	$targetpage = "expense_ent_listing.php"; 	//your file name  (the name of this file)
	$limit = 10;
...
//echo $page;die;
	...
	if($search112!=NULL)
	{
	$sql = "SELECT * FROM $tbl_name WHERE (head LIKE '%$search112%') OR (designation LIKE '%$search112%') OR (relate LIKE '%$search112%') OR (amount LIKE '%$search112%') ORDER BY id desc LIMIT $start, $limit";
	}
	else
  ...
  echo "<tr class='alternate-row'>";
				 ...
				 /* $len=strlen($ag_id);
				  ...
				  ?>
					...
					<a href="expense_ent_edit.php?id=<?php echo base64_encode(convert_uuencode($row['id']));?>" title="Edit" class="icon-1 info-tooltip"></a>
                 <?php  if($user==1 || $user==2) { ?> 
					<a href="delete.php?type=expense_ent&id=<?php echo base64_encode(convert_uuencode($row['id']));?>" onClick="if(!confirm('Are you sure, you want to delete this Information ?')){return false;}" title="Delete" class="icon-2 info-tooltip" ></a>
				<?php } ?>
