...
/*$query=mysql_query("SELECT book_alias,isbn FROM  item_master_list");
  ...
  if($f!=1)
   {
*/ 
$sql=mysql_query("UPDATE $tbl SET head='$head' ,parent='$parent'  where id='$id'")or die(mysql_error());
    if($sql)
...
<div id="page-heading">
  <h1>Edit Expense Head Master Information</h1>
...
<tr>
	...
	<td>
        ...
        <th valign="top">Head:</th>
			<td><input type="text" class="inp-form" name="head" id="head" value="<?php echo  $row['head']; ?>" required/></td>
          <th valign="top">Parent:</th>
			<td><input type="text" class="inp-form" name="parent" id="parent" value="<?php echo $row['parent']; ?>" required/></td> 
