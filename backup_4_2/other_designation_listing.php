...
<div id="content">
	...
	<form name="search113" method="post">
		<h1><span style="float:left">All Other Designation Listing </span> <span style="float: left; margin-left:70px"><input type="text" name="search112" class="searchboxs" value="<?php if($_REQUEST['all']) echo" "; ?>"></span><span style="float:left; margin-left:25px;"><input type="submit" name="search111" value="Search" class="searchboxs"></span></h1>
		</form>
	</div>
    <div style="width:100%;">
      <?php if($user==1){ ?>
    <div class="addin" style="width:auto; float:right;">
    	<span><a href="add_other_designation.php" style="color:#FFFFFF;">Add New Other Designation</a></span>
    </div>
	<div class="addin" style="width:auto; float:right;">
    	<span><a href="other_designation_listing.php" style="color:#FFFFFF;">Show All</a></span>
    </div>
	<?php }?>
	...
	*/
	...
	$tbl_name="other_designation_list";		//your table name
	...
	*/
	if($search112!=NULL)
	{
	$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE (designation LIKE '%$search112%')";
	}
	else
	...
	$targetpage = "other_designation_listing.php"; 	//your file name  (the name of this file)
	$limit = 10;
...
//echo $page;die;
	...
	if($search112!=NULL)
	{
	$sql = "SELECT * FROM $tbl_name WHERE (designation LIKE '%$search112%') ORDER BY id desc LIMIT $start, $limit";
	}
	else
  ...
  echo "<tr class='alternate-row'>";
				 ...
				 /* $len=strlen($ag_id);
				  ...
				  <td><?php echo $count; ?></td>
					...
					<a href="other_designation_edit.php?id=<?php echo base64_encode(convert_uuencode($row['id']));?>" title="Edit" class="icon-1 info-tooltip"></a>
                  
					<a href="delete.php?type=other_designation&id=<?php echo base64_encode(convert_uuencode($row['id']));?>" onClick="if(!confirm('Are you sure, you want to delete this Information ?')){return false;}" title="Delete" class="icon-2 info-tooltip" ></a>
