...
function update()
 {
	...
	$category=strtoupper($_REQUEST['category']);
	$name=strtoupper($_REQUEST['name']); 
	$name12=strtoupper($_REQUEST['name12']); 	
	$date = date('Y-m-d H:i:s'); 
	//echo'</pre>';print_r($encrypt);die;
	$sq=mysql_query("UPDATE school_designation SET category='$category', name='$name' where id='$id'") or die(mysql_error());
	$sq=mysql_query("UPDATE teacher_master_list SET designation='$name' where designation='$name12'") or die(mysql_error());
	//echo'</pre>';print_r($sq);die;
	if($sq){
	     header('location:designation_listing.php');
	    }
		else{ header('location:designation_info_edit.php?id='.$id);}  
 
  }

//end of function
	$result=mysql_query("SELECT name FROM school_designation WHERE (category = '$category') AND (id= '$id') ") or die(mysql_error());
	...
	if($row['name']==$name)
	  ...
	  {
	  $query=mysql_query("SELECT name FROM  school_designation WHERE (category = '$category')");
...
$result=mysql_query("SELECT * FROM school_designation WHERE id= '$id' ") or die(mysql_error());
$row = mysql_fetch_array($result);
...
<div id="page-heading">
  <h1>Edit Designation Information</h1>
