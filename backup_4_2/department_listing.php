...
<div id="content">
    ...
    <div class="addin" style="width:auto; float:right;">
    	<span><a href="department.php" style="color:#FFFFFF;">Add New Department</a></span>
    </div>
	...
	<td> 
         <select name="searchby1" class="search_by" >
				...
				<option <?php if($searchby1=="user") echo 'selected="selected"';?>VALUE="user">User ID</option>	                      
        </select>
      </td> 
	...
	<td> 
         <select name="searchby2" class="search_by">
				...
				<option <?php if($searchby2=="user") echo 'selected="selected"';?>VALUE="user">User ID</option>	                                          
        </select>
      </td>
				...
				<tr>
                   ...
                   <!-- <th class="table-header-repeat line-left"><a>Profile Image</a></th>-->
					...
					<th class="table-header-repeat line-left minwidth-1"><a>Name</a>	</th>
                     <?php //if($user==1){ ?>
					<th class="table-header-repeat line-left minwidth-1"><a>Department</a></th>
                    <?php //}?>
					<th class="table-header-repeat line-left"><a>Designation</a></th>
                    <th class="table-header-repeat line-left"><a>Email ID</a></th>
					  <?php //if($user==1){ ?>
					<th class="table-header-repeat line-left"><a>User ID</a></th> 
	...
	*/
	...
	$targetpage = "department_listing.php"; 	//your file name  (the name of this file)
	$limit = 10;
...
if ($i % 2 == 0) {
  ...
  echo "<tr class='alternate-row'>";
				  ...
				  $user_id=$row['user'];
				  $mobile="";
				  if($row['phone1']!="") $mobile=$row['phone1'].',';
				  if($row['mobile1']!="") $mobile.=$row['mobile1'].',';
				  if($row['mobile2']!="") $mobile.=$row['mobile2'];
				  //For select dan_user name
				  $query21=mysql_query("SELECT user_name FROM  `dan_users` WHERE id='$user_id' ")or die (mysql_error());
				  $data21=mysql_fetch_assoc($query21);
				  $user_name=$data21['user_name'];
				  //End
				  ?>
                  ...
                  <td align="center"><a href="book_sampling_department.php?sid=<?php echo base64_encode(convert_uuencode($row['id']));?>&type=<?php echo base64_encode(convert_uuencode($tbl_name));?>" title="Teacher Master"><img src="images/sicon.png"/></a></td>					<td class="options-width">
                    
					<a href="department_edit.php?id=<?php echo base64_encode(convert_uuencode($row['id']));?>" title="Edit" class="icon-1 info-tooltip"></a>
                  <?php  if($user==1 || $user==2) { ?>
					<a href="delete.php?type=department&id=<?php echo base64_encode(convert_uuencode($row['id']));?>" onClick="if(!confirm('Are you sure, you want to delete this Department Details ?')){return false;}" title="Delete" class="icon-2 info-tooltip" ></a>
				<?php } ?>
