...
function update(){
	...
	$sq=mysql_query("UPDATE master_list SET board='$name' where board='$name12'");

	$sq=mysql_query("UPDATE workshop SET board='$name' where board='$name12'");		
	//echo'</pre>';print_r($sq);die;
	if($sq){
	     header('location:board_listing.php');
	    }
		else{ header('location:board_info_edit.php?id='.$id);}
  }
//End function

 
 $result=mysql_query("SELECT name FROM  school_info_list WHERE (category = 'BOARD') AND (id= '$id') ") or die(mysql_error());
...
if($row['name']==$name)
  ...
  {
  $query=mysql_query("SELECT name FROM  school_info_list WHERE (category = 'BOARD')");
  ...
  if($f!=1)
   { 
    ...
    {$error="Board Detail must be unique";}
...
<tr>
   ...
   <th valign="top">Board Details:</th>
			<td>
