...
<div id="content">
	...
	<div id="page-heading">
		<form method="post">
		<h1><span style="float:left">Transport Details </span><span style="float: left; margin-left:11px"><input type="text" name="searchCity"  class="searchboxs" style=" border-radius:5px"  placeholder="Enter city name"/></span> <span style="float: left; margin-left:8px"><input type="text" name="searchState" class="searchboxs" style=" border-radius:5px" placeholder="Enter state name"/></span><span style="float:left; margin-left:25px;"><input type="submit" name="search111" value="Search" style=" border-radius:5px"></span></h1>
	   </form>
	</div>
    <div style="width:100%;">
     
    <div class="addin" style="width:auto; float:right;">
    	<span><a href="add_transport.php" style="color:#FFFFFF;">Add Transport Details</a></span>
    </div>
    <div class="addin" style="width:auto; float:right;" align="center"> 
   <span ><a href="transport_details_listing.php" style="color:#FFFFFF;">Show All</a></span> 
   </div>
	...
	<tr>
		...
		<div id="content-table-inner">
			...
			<div id="table-content">
				...
				<tr>
                   ...
                   <!-- <th class="table-header-repeat line-left"><a>Profile Image</a></th>-->
					...
					<th class="table-header-repeat line-left minwidth-1"><a>State</a></th>
                     <th class="table-header-repeat line-left"><a>Contact No</a></th>
					<th class="table-header-repeat line-left minwidth-1"><a>Email ID</a></th>
                ...
                <tbody>
                <?php
                $searchCity=mysql_real_escape_string($_REQUEST['searchCity']);
                $searchState=mysql_real_escape_string($_REQUEST['searchState']);
	/*  
		Place code to connect to your DB here.
	*/
	//include('config.php');	// include your code to connect to DB.

	$tbl_name="transport_list";		//your table name
	...
	*/
	...
	{
	$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE (city LIKE '%$searchCity%') ";
	}
   elseif(($searchCity==NULL) && ($searchState!=NULL))
    {
	$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE (state LIKE'%$searchState%')";
	}
   elseif(($searchCity!=NULL) && ($searchState!=NULL))
    {
	$query = "SELECT COUNT(*) as num FROM $tbl_name WHERE (city LIKE'%$searchCity%') AND (state LIKE'%$searchState%') ";
	}
	...
	$targetpage = "transport_details_listing.php"; 	//your file name  (the name of this file)
	$limit = 10;
...
//echo $page;die;
	...
	if($page) 
		...
		$start = 0;								//if no page var is given, set start to 0  OR  OR ((city='$searchCity')  AND (state='$searchState'))
	
	/* Get data. */
	if(($searchCity!=NULL) && ($searchState==NULL))
	{
	$sql = "SELECT * FROM $tbl_name WHERE (city LIKE'%$searchCity%')   ORDER BY id desc LIMIT $start, $limit";
	}
   elseif(($searchCity==NULL) && ($searchState!=NULL))
    {
	$sql = "SELECT * FROM $tbl_name WHERE (state LIKE'%$searchState%')   ORDER BY id desc LIMIT $start, $limit";
	}
   elseif(($searchCity!=NULL) && ($searchState!=NULL))
    {
	$sql = "SELECT * FROM $tbl_name WHERE (city LIKE '%$searchCity%') AND (state LIKE '%$searchState%')  ORDER BY id desc LIMIT $start, $limit";
	}
   else
   {
  ...
  echo "<tr class='alternate-row'>";
				 ...
				 /* $len=strlen($ag_id);
				  ...
				  <td><?php echo $count; ?></td>
					...
					<a href="transport_edit.php?id=<?php echo base64_encode(convert_uuencode($row['id']));?>" title="Edit" class="icon-1 info-tooltip"></a>
                  <?php  if($user==1 || $user==2) { ?>  	
					<a href="delete.php?type=transport&id=<?php echo base64_encode(convert_uuencode($row['id']));?>" onClick="if(!confirm('Are you sure, you want to delete this Information ?')){return false;}" title="Delete" class="icon-2 info-tooltip" ></a>			
